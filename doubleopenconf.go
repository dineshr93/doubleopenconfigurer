...
func main() {
	...
	go func(folderPath string) {
		...
		doubleOpenURL := "https://github.com/doubleopen-project/meta-doubleopen.git"
		if _, err:= os.Stat(doubleOpenRepo); errors.Is(err, fs.ErrNotExist){
	
			fmt.Println(doubleOpenRepo," git Repo doesnt exist in",folderPath,". Cloning from https://github.com/doubleopen-project/meta-doubleopen.git")
